...
// mongoDB Connection
const mongoURI = "mongodb+srv://Pranabbhowmik:Pranab123@cluster0.pwhfbbg.mongodb.net/online-voting-system";
...
app.post("/login", async (req, res) => {
    ...
    try {
        const candidate = await Voter.findOne({ aadhar_id: aadhar, voter_id: voter });
...
app.post("/vote", async (req, res) => {
    try {
        ...
        if (check) {
            return res.status(401).json({ error: true, message: 'vote is already given by voter' })
        }
